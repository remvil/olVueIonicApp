...
// 	if (error.response) return error.response.data ? formatResponseData(error, code, handledAt) : formatErrorCode(error, code, handledAt);
// 	else return formatInternalError(error, code, handledAt);
// };

// export const apiErrorHandler = (err: ErrorType, _req: Request, res: Response, _next: NextFunction) => {
// 	if (err.response) {
// 		Logger.writeException(err.response.data, "001-Unknown", "apiErrorHandler");
// 		return res.status(err.response.status).json({message: err.message, response: err.response.data});
// 	} else {
...
// export const asyncErrWrapper = (fun: FunType) => (req: Request, res: Response, next: NextFunction) => {
// 	Promise.resolve(fun(req, res, next)).catch(next);
// };

// const logResposeData = (handledAt: string | undefined, error: any, code: string) =>
// 	handledAt ? Logger.writeException(new Error(error), code, handledAt) : Logger.writeException(error, code);

// const logResponseCode = (handledAt: string | undefined, error: any, code: string) =>
// 	handledAt
...
// const logSimpleError = (handledAt: string | undefined, error: string, code: string) =>
// 	handledAt ? Logger.writeException(new Error(error), code, handledAt) : Logger.writeException(new Error(error), code);

// const formatInternalError = (error: any, code: string, handledAt: string | undefined): ErrorPayload => {
